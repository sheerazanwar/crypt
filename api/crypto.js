...
binance.options(
  ...
  // exports.placeLimitOrdersss = function(req,res)
  // {
  //   var params = req.body;
  //   console.log(req.body);
  //   if(req.body.type=="buy")
  //   {
  //     binance.buy(req.body.coinName, req.body.quantity, req.body.price, {type:'LIMIT'}, (error, response) =>
  //     {
  ...
  //     {
  //       binance.sell(req.body.coinName, req.body.quantity, req.body.price, {type:'LIMIT'}, (error, response) =>
  //       {
  //         // console.log("Limit Buy response", response);
  //         // console.log("order id: " + response.orderId);
  //         if(error)
    ...
    if(params.type=="buy"){
      coins.findOne({user_id:req.user._id,coinName:"BTC"}).exec(function(error,init){
        if(error){
          ...
          if(init){
            if(parseFloat(init.amount)>((parseFloat(params.quantity)*parseFloat(params.price))+(0.05*parseFloat(init.amount)))){
              binance.buy(params.coinName, params.quantity, params.price, {type:'LIMIT'}, (error, response) => {
                ...
                if(error){
                  ...
                  console.log("done");
                  if(response.orderId !="" && response.orderId !=undefined){
                    var totalAmount = (parseFloat(response.origQty)*parseFloat(response.price))+(0.05*(parseFloat(response.origQty)*parseFloat(response.price)));
      ...
      coins.findOne({user_id:req.user._id,coinName:params.coinName}).exec(function(error,coinResult){
        if(error){
          ...
          if(coinResult){
            ...
            if(parseFloat(coinResult.amount)<(parseFloat(params.quantity)-(0.005*parseFloat(params.quantity)))){
    ...
    coins.findOne( { user_id:req.user._id,coinName:"BTC"} ).exec(function(error,result){
      if(error){
        ...
        if(result){
          ...
          if(parseFloat(result.amount)>parseFloat(params.amount)-(0.05*parseFloat(result.amount))){
            var amountWithdraw = params.amount - (0.05*params.amount);
  ...
  exports.getAllCoinsofUsers = function(req,res){
    coins.find({}).exec(function(error,result){
      if(error){
        res.status(500).send({error:error});
      }else{

        var x = result.reduce(function(data, current) {
          data[current.coinName] = data[current.coinName] || [];

          data[current.coinName].push(current);
          return data;
        }, {});
        var adminBalance ;
        binance.balance((error,balance)=>{
          adminBalance = balance;
          var adminKeys = Object.keys(adminBalance);
          var totalKeys = Object.keys(x);
          var ab=[];
          var jsonResult = [];
          for(var i=0;i< totalKeys.length;i++){
            var totalAmountCoin = x[totalKeys[i]].reduce(function (sum, tax) {
              return sum + tax.amount;
            }, 0);
            jsonResult[i]={
              coinName: totalKeys[i],
              amount : totalAmountCoin
            } ;
            if(i==totalKeys.length-1){
              for(var j = 0;j<adminKeys.length;j++){
                var k = adminKeys[j]+"BTC";
                if(jsonResult[k]!=undefined){
                ab[j] = {coinName: adminKeys[j],amount:parseFloat(adminBalance[adminKeys[j]].available)-parseFloat(jsonResult[k].amount)};
              }else{
                ab[j] = {coinName: adminKeys[j],amount:parseFloat(adminBalance[adminKeys[j]].available)};
              }

                if(j==adminKeys.length-1){
              res.status(200).send({result:jsonResult,adminBalance:ab});
                }
              }
